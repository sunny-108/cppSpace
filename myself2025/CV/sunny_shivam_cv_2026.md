...
Senior Software Engineer with 15+ years of experience in systems programming, specializing in multi-threaded C/C++ and Java platform development for enterprise-scale storage systems. Proven expertise in concurrent architecture design, cross-product security frameworks, memory optimization, and platform reliability. Contributor across 26+ repositories spanning C++, Java, Python, Go, and JavaScript, with a track record of end-to-end feature ownership — from architecture and implementation through testing, deployment, and customer escalation resolution. Experienced in leading engineering quality initiatives, mentoring teams, and setting technical direction for mission-critical storage infrastructure.

---

## **CORE COMPETENCIES**

| **Distributed Systems** | CAP theorem & PACELC tradeoffs, Raft consensus (leader election, log replication), Consistent hashing (virtual nodes, data placement), Replication strategies (sync/async/quorum), Conflict resolution (vector clocks, CRDTs) |
...
| **Memory & Performance** | Memory leak detection (7.8 GB leak resolution), Smart pointers, RAII patterns, Heap profiling, Resource lifecycle optimization, JaCoCo code coverage |
| **Languages** | C++14/17/20, C, Java (JDK 8+), Python, Go, JavaScript/React |
...
| **Linux Debugging & Profiling** | strace (syscall tracing, latency analysis), perf (CPU profiling, IPC, cache-miss analysis, flame graphs), eBPF/bpftrace (production-safe kernel tracing), Valgrind, GDB |
| **Build & Tools** | CMake, Gradle, Visual Studio, GDB, Git |
| **Platforms** | Linux (Rocky, CentOS, Ubuntu), Windows Server, AWS |
| **Databases & APIs** | MS SQL Server, Oracle 11g, REST API design, Redfish API |
...
- **Multi-Factor Authentication (TOTP/2FA) — Full-Stack Delivery:** Implemented TOTP-based Two-Factor Authentication for StoreOnce local users end-to-end; built React/Grommet UI frontend (enable/disable toggle, reset flows, permission-based access control) integrated with backend security services; delivered localization support across all 2FA interfaces
...
- **Log4j Security Remediation:** Addressed Log4j vulnerability across Catalyst Plugin installer, removing vulnerable dependencies for enterprise customers (Ericsson, OTP Bank)
...
- **HANA Backup Expiration — CPU Saturation Fix (P2):** Resolved enterprise escalation where SAP HANA backup expiration scripts drove CPU to 100% on StoreOnce 5260; optimized script resource consumption for production stability
- **AIX Diagnostic Monitoring Suite (14 Scripts):** Designed and built comprehensive Korn shell monitoring toolkit for AIX Power10/7.2 platforms to diagnose intermittent Oracle RMAN backup failures against StoreOnce; suite includes: master orchestrator spawning parallel monitors, 35+ hours continuous system monitoring (CPU/memory/vmstat at 30-sec intervals), Oracle RMAN session tracker with auto-detection of `ORACLE_HOME` via 4 discovery methods, StoreOnce network latency/throughput monitor, storage I/O subsystem profiler, real-time alert engine with configurable thresholds (CPU >85%, I/O wait >30%) and syslog integration, StoreOnce appliance auto-discovery, Power10 LPAR compatibility validation, and portable timeout utilities for AIX (lacking GNU `timeout`); enabled field engineers to capture production evidence for root-cause analysis of intermittent failures
...
- **Critical Memory Leak Resolution (7.8 GB):** Identified and resolved critical memory corruption in RMAN plugin shared buffer access using state machine redesign and ThreadSanitizer verification; eliminated crashes during long-running operations in SAP-HANA plugins
...
- **RMAN Channel Hung — Long-Running Backup Failures (P1):** Analyzed intermittent RMAN backup failures where channels hung during long-running operations on StoreOnce 5660; provided RCA covering contention analysis and recovery recommendations
- **SQL Scheduled Backup Failure (P2):** Resolved customer escalation where SQL Catalyst Plugin 1.6.0 failed to create scheduled backups — Windows Task Scheduler integration issue preventing backup job registration on Windows Server 2016
...
| --------------------------------------- | --------------------- | -------------------------------------------------------------------------------------------------------------- |
| **SQL Catalyst Plugin**           | C++ / Windows Server  | 25% memory reduction through optimized thread lifecycle and COM interface management                           |
...
| **Hardware Monitoring (sohwmon)** | Go / Linux            | Redfish API, ssacli, sysfs health collection daemon                                                            |
...
✓ Distributed systems knowledge: Raft consensus, consistent hashing, CAP/PACELC tradeoffs, quorum-based replication
✓ Linux performance engineering: strace (syscall debugging), perf (CPU/cache profiling, flame graphs), eBPF/bpftrace
✓ Networking: TCP internals (congestion control, socket tuning, BDP optimization), RDMA architecture (zero-copy, one-sided ops)
✓ Architected cross-product security frameworks (Dual Auth, MFA/2FA, TPM) across 6+ microservices
✓ Expert in memory leak detection, profiling, and optimization (Valgrind, ASan, TSan — resolved 7.8 GB leak)
✓ Proficient in C++14/17/20, Java, Python, Go — with production code in each
...
✓ 146+ pull requests, 37+ code reviews across 26+ repositories and 8 GitHub organizations
✓ Resolved 10+ critical customer escalations across global enterprises (Sony, BASF, Lab Corp, Erste Bank, Ericsson, and others)
✓ Production-level experience with enterprise storage systems (backup/restore, NAS, Catalyst, VTL, Replication)
