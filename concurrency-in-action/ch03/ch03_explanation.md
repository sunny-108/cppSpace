...
## 1. Problems with Sharing Data

When threads share data, several distinct problems can occur. Understanding each type helps you choose the right solution.

### 1.1 Race Conditions

**Simple Explanation:**
Imagine two people trying to edit the same Google Doc at the exact same time. They might overwrite each other's changes. A race condition is a timing-dependent bug where the program's behavior depends on which thread "wins the race" to execute first.
...
### 1.2 Data Races (Undefined Behavior)

**Simple Explanation:**
A data race is more severe than a race condition. It occurs when two threads access the same memory location simultaneously, at least one is writing, and there's no synchronization. This causes **undefined behavior** - your program might crash, produce garbage, or appear to work (the worst case, because the bug is hidden).

**Key Difference:**
- **Race Condition**: Logic error, wrong result, but behavior is defined
- **Data Race**: Undefined behavior at the language level - anything can happen
...
void writer() {
    for (int i = 0; i < 1000000; ++i) {
        shared_value = i; // Writing without synchronization
    }
}

void reader() {
    for (int i = 0; i < 1000000; ++i) {
        // Reading while writer is modifying - DATA RACE!
        // Might see torn reads, crashes, or corrupted values
...
int main() {
    ...
    t1.join();
    t2.join();
    return 0; // Undefined behavior throughout
}
```

---

### 1.3 Invariant Violations

**Simple Explanation:**
An invariant is a condition that should always be true. For example, in a bank account, "balance = deposits - withdrawals" is an invariant. When multiple threads access a data structure, one thread might observe it in an inconsistent state while another is modifying it.

**Example: Breaking Invariants**

```cpp
class BankAccount {
    int checking = 1000;
    int savings = 2000;
    // INVARIANT: total_balance = checking + savings = 3000
  
public:
    void transfer_to_savings(int amount) {
        checking -= amount;        // Step 1: invariant broken temporarily!
        // Another thread could read here and see wrong total
        savings += amount;         // Step 2: invariant restored
    }
  
    int total_balance() {
        return checking + savings; // Might see inconsistent state!
    }
};

// Thread 1: transfer_to_savings(100)
// Thread 2: total_balance() called between step 1 and 2
// Result: Thread 2 sees total < 3000 (invariant violated!)
```

---

### 1.4 Deadlock (Preview)

**Simple Explanation:**
Deadlock occurs when two or more threads are waiting for each other to release resources, creating a circular dependency. All threads are stuck forever.
...
m1.lock();  // Gets m1
// ...
m2.lock();  // Waits for m2 (held by Thread 2)

// Thread 2:
m2.lock();  // Gets m2
// ...
m1.lock();  // Waits for m1 (held by Thread 1)

// DEADLOCK! Both threads wait forever
```

**Note:** Deadlock is covered in detail in Section 3 below.

---

### 1.5 Memory Ordering Issues

**Simple Explanation:**
Modern CPUs and compilers reorder operations for performance. Without proper synchronization, one thread might see changes in a different order than they were written.

**Example: Reordering Problem**
...
// Problem: CPU might reorder Thread 1's operations:
// ready = true;     (executed first due to optimization)
// data = 42;        (executed second)
// 
// Result: Thread 2 sees ready=true but data is still uninitialized!
```

**Note:** Proper mutex usage (covered in Section 2) automatically handles memory ordering.
...
| --------------------- | -------------------------------- | --------------------- |
| **Race Condition**    | Wrong results, logic errors      | High                  |
| **Data Race**         | Undefined behavior, crashes      | Critical              |
| **Invariant Violation** | Inconsistent state observed    | High                  |
| **Deadlock**          | Program hangs forever            | Critical              |
| **Memory Ordering**   | Stale/reordered data observed    | High                  |

**The Solution:** All these problems are solved by proper synchronization using mutexes, atomics, and other mechanisms covered in the following sections.
